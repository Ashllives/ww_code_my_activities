...
while is_done != 'y':
    ...
    # checks if the item is already existing
    if item in cart:
        cart[item] += 1
    else:
        cart[item] = 1
    ...
    # once done punching items, print check as:
    if is_done == 'y':
        total = 0
        print('***** PRINTING YOUR RECEIPT *****')
        for key, value in cart.items():
            if cart[key] > 1:
                print(key + '    ' + str(value) + '    ' + str(products[key] * cart[key]))
                total += products[key] * cart[key]
            else:
                print(key + '    ' + str(value) + '    ' + str(products[key]))
                total += products[key] 
